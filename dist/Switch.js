...
function isCaseComponents({ children }, propName, componentName) {
  ...
  React.Children.forEach(children, child => {
    if (!isAllowedChild(child)) {
      error = new Error(`${componentName} children should be of type 'Case' or 'Default'. Received ${child.type}`);
    } else if (isDefaultComponent(child)) {
      if (containsDefault) {
        error = new Error(`${componentName} expected exactly one Default as a child. Received too many.`);
      }
      containsDefault = true;
    }
  });
  if (!containsDefault) {
    error = new Error(`${componentName} expected exactly one Default as a child. Received zero.`);
  }
  return error;
}

function isMatchFor(arg) {
  return caseComponent => caseComponent.props.match === arg || caseComponent.props.test && caseComponent.props.test(arg);
}

const experimentalVariation = (children, arg) => children.filter(isCaseComponent).find(isMatchFor(arg));
...
export default function Switch({ children, arg }) {
  ...
  return React.Children.only(experimentalVariation(cases, arg) || defaultVariation(cases));
}
